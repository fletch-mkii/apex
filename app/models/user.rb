...
class User < ActiveRecord::Base
  ...
  def set_location(ip)
    # response = HTTPClient.new.get("http://freegeoip.net/json/#{ip}") #real code to be implemented in production
    response = HTTPClient.new.get("http://freegeoip.net/json/50.241.127.209") #test code because localhost ip doesn't mesh with api
